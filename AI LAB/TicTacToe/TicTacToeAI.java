...
public class TicTacToeAI {
    ...
    public static int max(Board board) {
        ...
        for (Position p : positions) { // calculate the score for all the free spots
            branchesVisited++; // Increment counter for each branch explored

            Board b = new Board(board, p, PlayerSign.Cross);
            int move = min(b);// current player is X; hence for next move(i.e O aka Minimizing player we are
                              // finding min)
            if (move > best)
                best = move;
        }
    ...
    public static int min(Board board) { // Minimizing player wants the lowest score for itself thats the best for it
        GameState gameState = board.getGameState();
        if (gameState == GameState.CircleWin)
            return 1;
        else if (gameState == GameState.CrossWin)
        ...
        for (Position p : positions) { // calculate the score for all the free spots
            ...
            int move = max(b); // current player is O; hence for next move(i.e X aka Maximizing player we are
                               // finding max)
            if (move < best)
                best = move;
        }
